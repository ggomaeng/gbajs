...
var require$$1 = require('pngjs');
var require$$0 = require('buffer-dataview');
...
function hex$1(number, leading, usePrefix) {
...
var Serializer$2 = {
	...
	prefix: function(value) {
		return new Blob([Serializer$2.pack(value.size || value.length || value.byteLength), value], { type: Serializer$2.TYPE });
	},
	...
	serialize: function(stream) {
		...
		for (i in stream) {
			if (stream.hasOwnProperty(i)) {
				var tag;
				var head = Serializer$2.prefix(i);
				var body;
				switch (typeof(stream[i])) {
				case 'number':
					tag = Serializer$2.TAG_INT;
					body = Serializer$2.pack(stream[i]);
					break;
				case 'string':
					tag = Serializer$2.TAG_STRING;
					body = Serializer$2.prefix(stream[i]);
					break;
				case 'object':
					if (stream[i].type == Serializer$2.TYPE) {
						tag = Serializer$2.TAG_BLOB;
						body = stream[i];
					} else {
						tag = Serializer$2.TAG_STRUCT;
						body = Serializer$2.serialize(stream[i]);
					}
					break;
				case 'boolean':
					tag = Serializer$2.TAG_BOOLEAN;
					body = Serializer$2.pack8(stream[i]);
					break;
				default:
				...
				parts.push(Serializer$2.pack8(tag));
				parts.push(head);
				parts.push(body);
			}
		}
		parts.unshift(Serializer$2.pack(size));
	...
	deserialize: function(blob, callback) {
		...
		reader.onload = function(data) {
			callback(Serializer$2.deserealizeStream(new DataView(data.target.result), new Serializer$2.pointer));
		};
	...
	deserealizeStream: function(view, pointer) {
		...
		while (pointer.mark() < remaining) {
			...
			var body;
			switch (tag) {
			case Serializer$2.TAG_INT:
				body = view.getUint32(pointer.advance(4), true);
				break;
			case Serializer$2.TAG_STRING:
				body = pointer.readString(view);
				break;
			case Serializer$2.TAG_STRUCT:
				body = Serializer$2.deserealizeStream(view, pointer);
				break;
			case Serializer$2.TAG_BLOB:
				var size = view.getUint32(pointer.advance(4), true);
				body = view.buffer.slice(pointer.advance(size), pointer.advance(0));
				break;
			case Serializer$2.TAG_BOOLEAN:
	...
	deserializePNG: function(blob, callback) {
		...
		reader.onload = function(data) {
			...
			newBlob = new Blob(data.map(function (byte) {
				...
				array[0] = byte;
				return array;
			}), { type: Serializer$2.TYPE});
			Serializer$2.deserialize(newBlob, callback);
		};
		reader.readAsDataURL(blob);
	}
};

Serializer$2.pointer.prototype.advance = function(amount) {
	var index = this.index;
	this.index += amount;
	return index;
};

Serializer$2.pointer.prototype.mark = function() {
	return this.index - this.top;
};

Serializer$2.pointer.prototype.push = function() {
	this.stack.push(this.top);
	this.top = this.index;
};

Serializer$2.pointer.prototype.pop = function() {
	this.top = this.stack.pop();
};

Serializer$2.pointer.prototype.readString = function(view) {
...
util$1.inherit = inherit$4;
util$1.hex = hex$1;
util$1.Serializer = Serializer$2;

var ARMCoreArm$1 = function (cpu) {
	this.cpu = cpu;
...
ARMCoreArm$1.prototype.constructAddressingMode1ASR = function(rs, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		++cpu.cycles;
...
ARMCoreArm$1.prototype.constructAddressingMode1Immediate = function(immediate) {
	var cpu = this.cpu;
	return function() {
...
ARMCoreArm$1.prototype.constructAddressingMode1ImmediateRotate = function(immediate, rotate) {
	var cpu = this.cpu;
	return function() {
...
ARMCoreArm$1.prototype.constructAddressingMode1LSL = function(rs, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		++cpu.cycles;
...
ARMCoreArm$1.prototype.constructAddressingMode1LSR = function(rs, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		++cpu.cycles;
...
ARMCoreArm$1.prototype.constructAddressingMode1ROR = function(rs, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		++cpu.cycles;
...
ARMCoreArm$1.prototype.constructAddressingMode23Immediate = function(instruction, immediate, condOp) {
	var rn = (instruction & 0x000F0000) >> 16;
	return this.addressingMode23Immediate[(instruction & 0x01A00000) >> 21](rn, immediate, condOp);
};

ARMCoreArm$1.prototype.constructAddressingMode23Register = function(instruction, rm, condOp) {
	var rn = (instruction & 0x000F0000) >> 16;
	return this.addressingMode23Register[(instruction & 0x01A00000) >> 21](rn, rm, condOp);
};

ARMCoreArm$1.prototype.constructAddressingMode2RegisterShifted = function(instruction, shiftOp, condOp) {
	var rn = (instruction & 0x000F0000) >> 16;
	return this.addressingMode2RegisterShifted[(instruction & 0x01A00000) >> 21](rn, shiftOp, condOp);
};

ARMCoreArm$1.prototype.constructAddressingMode4 = function(immediate, rn) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreArm$1.prototype.constructAddressingMode4Writeback = function(immediate, offset, rn, overlap) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
...
ARMCoreArm$1.prototype.constructADC = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreArm$1.prototype.constructADCS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructADD = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructADDS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructAND = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructANDS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructB = function(immediate, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		if (condOp && !condOp()) {
...
ARMCoreArm$1.prototype.constructBIC = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructBICS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructBL = function(immediate, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		if (condOp && !condOp()) {
...
ARMCoreArm$1.prototype.constructBX = function(rm, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		if (condOp && !condOp()) {
...
ARMCoreArm$1.prototype.constructCMN = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructCMP = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructEOR = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructEORS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructLDM = function(rs, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	var mmu = cpu.mmu;
	return function() {
...
ARMCoreArm$1.prototype.constructLDMS = function(rs, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	var mmu = cpu.mmu;
	return function() {
...
ARMCoreArm$1.prototype.constructLDR = function(rd, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructLDRB = function(rd, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructLDRH = function(rd, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructLDRSB = function(rd, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructLDRSH = function(rd, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructMLA = function(rd, rn, rs, rm, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructMLAS = function(rd, rn, rs, rm, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructMOV = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructMOVS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructMRS = function(rd, r, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructMSR = function(rm, r, instruction, immediate, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
...
ARMCoreArm$1.prototype.constructMUL = function(rd, rs, rm, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructMULS = function(rd, rs, rm, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructMVN = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructMVNS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructORR = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructORRS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructRSB = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructRSBS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructRSC = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructRSCS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructSBC = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructSBCS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructSMLAL = function(rd, rn, rs, rm, condOp) {
	var cpu = this.cpu;
...
ARMCoreArm$1.prototype.constructSMLALS = function(rd, rn, rs, rm, condOp) {
	var cpu = this.cpu;
	var SHIFT_32 = 1/0x100000000;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreArm$1.prototype.constructSMULL = function(rd, rn, rs, rm, condOp) {
	var cpu = this.cpu;
	var SHIFT_32 = 1/0x100000000;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreArm$1.prototype.constructSMULLS = function(rd, rn, rs, rm, condOp) {
	var cpu = this.cpu;
	var SHIFT_32 = 1/0x100000000;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreArm$1.prototype.constructSTM = function(rs, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	var mmu = cpu.mmu;
	return function() {
...
ARMCoreArm$1.prototype.constructSTMS = function(rs, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	var mmu = cpu.mmu;
	return function() {
...
ARMCoreArm$1.prototype.constructSTR = function(rd, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		if (condOp && !condOp()) {
...
ARMCoreArm$1.prototype.constructSTRB = function(rd, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		if (condOp && !condOp()) {
...
ARMCoreArm$1.prototype.constructSTRH = function(rd, address, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		if (condOp && !condOp()) {
...
ARMCoreArm$1.prototype.constructSUB = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructSUBS = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructSWI = function(immediate, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		if (condOp && !condOp()) {
...
ARMCoreArm$1.prototype.constructSWP = function(rd, rn, rm, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructSWPB = function(rd, rn, rm, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructTEQ = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructTST = function(rd, rn, shiftOp, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch32(gprs[cpu.PC]);
...
ARMCoreArm$1.prototype.constructUMLAL = function(rd, rn, rs, rm, condOp) {
	var cpu = this.cpu;
	var SHIFT_32 = 1/0x100000000;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreArm$1.prototype.constructUMLALS = function(rd, rn, rs, rm, condOp) {
	var cpu = this.cpu;
	var SHIFT_32 = 1/0x100000000;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreArm$1.prototype.constructUMULL = function(rd, rn, rs, rm, condOp) {
	var cpu = this.cpu;
	var SHIFT_32 = 1/0x100000000;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreArm$1.prototype.constructUMULLS = function(rd, rn, rs, rm, condOp) {
	var cpu = this.cpu;
	var SHIFT_32 = 1/0x100000000;
	var gprs = cpu.gprs;
	return function() {
...
var arm = ARMCoreArm$1;

var ARMCoreThumb$1 = function (cpu) {
	this.cpu = cpu;
};

ARMCoreThumb$1.prototype.constructADC = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreThumb$1.prototype.constructADD1 = function(rd, rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructADD2 = function(rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructADD3 = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructADD4 = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructADD5 = function(rd, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructADD6 = function(rd, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructADD7 = function(immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructAND = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructASR1 = function(rd, rm, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructASR2 = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructB1 = function(immediate, condOp) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructB2 = function(immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructBIC = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructBL1 = function(immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructBL2 = function(immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructBX = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructCMN = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructCMP1 = function(rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructCMP2 = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructCMP3 = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructEOR = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLDMIA = function(rn, rs) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLDR1 = function(rd, rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLDR2 = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLDR3 = function(rd, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLDR4 = function(rd, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLDRB1 = function(rd, rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		var n = gprs[rn] + immediate;
...
ARMCoreThumb$1.prototype.constructLDRB2 = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLDRH1 = function(rd, rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		var n = gprs[rn] + immediate;
...
ARMCoreThumb$1.prototype.constructLDRH2 = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLDRSB = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLDRSH = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLSL1 = function(rd, rm, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLSL2 = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLSR1 = function(rd, rm, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructLSR2 = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructMOV1 = function(rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructMOV2 = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructMOV3 = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructMUL = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructMVN = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructNEG = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructORR = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructPOP = function(rs, r) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructPUSH = function(rs, r) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreThumb$1.prototype.constructROR = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructSBC = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructSTMIA = function(rn, rs) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreThumb$1.prototype.constructSTR1 = function(rd, rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		var n = gprs[rn] + immediate;
...
ARMCoreThumb$1.prototype.constructSTR2 = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreThumb$1.prototype.constructSTR3 = function(rd, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreThumb$1.prototype.constructSTRB1 = function(rd, rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		var n = gprs[rn] + immediate;
...
ARMCoreThumb$1.prototype.constructSTRB2 = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreThumb$1.prototype.constructSTRH1 = function(rd, rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		var n = gprs[rn] + immediate;
...
ARMCoreThumb$1.prototype.constructSTRH2 = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreThumb$1.prototype.constructSUB1 = function(rd, rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructSUB2 = function(rn, immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructSUB3 = function(rd, rn, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
ARMCoreThumb$1.prototype.constructSWI = function(immediate) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
...
ARMCoreThumb$1.prototype.constructTST = function(rd, rm) {
	var cpu = this.cpu;
	var gprs = cpu.gprs;
	return function() {
		cpu.mmu.waitPrefetch(gprs[cpu.PC]);
...
var thumb = ARMCoreThumb$1;

var inherit$3 = util$1.inherit;
var ARMCoreArm = arm;
var ARMCoreThumb = thumb;
...
ARMCore$1.prototype.resetCPU = function(startOffset) {
...
ARMCore$1.prototype.freeze = function() {
	return {
		'gprs': [
			this.gprs[0],
			this.gprs[1],
...
ARMCore$1.prototype.defrost = function(frost) {
	this.instruction = null;
...
ARMCore$1.prototype.fetchPage = function(address) {
...
ARMCore$1.prototype.loadInstructionArm = function(address) {
	var next = null;
...
ARMCore$1.prototype.loadInstructionThumb = function(address) {
	var next = null;
	this.fetchPage(address);
...
ARMCore$1.prototype.selectBank = function(mode) {
	switch (mode) {
	case this.MODE_USER:
...
ARMCore$1.prototype.switchExecMode = function(newMode) {
...
ARMCore$1.prototype.switchMode = function(newMode) {
...
ARMCore$1.prototype.hasSPSR = function() {
	return this.mode != this.MODE_SYSTEM && this.mode != this.MODE_USER;
};

ARMCore$1.prototype.raiseIRQ = function() {
	if (this.cpsrI) {
		return;
	}
...
ARMCore$1.prototype.raiseTrap = function() {
	var cpsr = this.packCPSR();
	var instructionWidth = this.instructionWidth;
...
ARMCore$1.prototype.badOp = function(instruction) {
	var func = function() {
...
ARMCore$1.prototype.generateConds = function() {
	var cpu = this;
	this.conds = [
		// EQ
		function() {
...
ARMCore$1.prototype.barrelShiftImmediate = function(shiftType, immediate, rm) {
	var cpu = this;
	var gprs = this.gprs;
...
ARMCore$1.prototype.compileArm = function(instruction) {
...
ARMCore$1.prototype.compileThumb = function(instruction) {
...
var BufferDataView = require$$0;
var inherit$2 = util$1.inherit;

function MemoryView$2(memory, offset) {
	inherit$2.call(this);
	this.buffer = memory;
	this.view = MemoryView$2.DataView(this.buffer, typeof(offset) === "number" ? offset : 0);
	this.mask = memory.byteLength - 1;
	this.resetMask();
}
MemoryView$2.DataView = function (buffer, byteOffset, byteLength) {
...
MemoryView$2.prototype.store8 = function(offset, value) {
	this.view.setInt8(offset & this.mask8, value);
};

MemoryView$2.prototype.store16 = function(offset, value) {
	this.view.setInt16(offset & this.mask16, value, true);
};

MemoryView$2.prototype.store32 = function(offset, value) {
	this.view.setInt32(offset & this.mask32, value, true);
};

MemoryView$2.prototype.invalidatePage = function(address) {};

MemoryView$2.prototype.replaceData = function(memory, offset) {
	this.buffer = memory;
...
var memoryView = MemoryView$2;

var gpio = {};

function GameBoyAdvanceGPIO$1(core, rom) {
	this.core = core;
	this.rom = rom;
...
GameBoyAdvanceGPIO$1.prototype.store16 = function(offset, value) {
	switch (offset) {
	case 0xC4:
...
GameBoyAdvanceGPIO$1.prototype.outputPins = function(nybble) {
	if (this.readWrite) {
...
gpio.GameBoyAdvanceGPIO = GameBoyAdvanceGPIO$1;
gpio.GameBoyAdvanceRTC = GameBoyAdvanceRTC;

var savedata = {};

var MemoryView$1 = memoryView;

function SRAMSavedata$1(size) {
	MemoryView$1.call(this, new ArrayBuffer(size), 0);

	this.writePending = false;
}
SRAMSavedata$1.prototype = Object.create(MemoryView$1.prototype);

SRAMSavedata$1.prototype.store8 = function(offset, value) {
	this.view.setInt8(offset, value);
	this.writePending = true;
};

SRAMSavedata$1.prototype.store16 = function(offset, value) {
	this.view.setInt16(offset, value, true);
	this.writePending = true;
};

SRAMSavedata$1.prototype.store32 = function(offset, value) {
	this.view.setInt32(offset, value, true);
	this.writePending = true;
};

function FlashSavedata$1(size) {
	MemoryView$1.call(this, new ArrayBuffer(size), 0);
	...
	this.bank0 = MemoryView$1.DataView(this.buffer, 0, 0x00010000);
	if (size > 0x00010000) {
		this.id = this.ID_SANYO;
		this.bank1 = MemoryView$1.DataView(this.buffer, 0x00010000);
	} else {
...
FlashSavedata$1.prototype = Object.create(MemoryView$1.prototype);

FlashSavedata$1.prototype.load8 = function(offset) {
...
FlashSavedata$1.prototype.load16 = function(offset) {
	return (this.load8(offset) & 0xFF) | (this.load8(offset + 1) << 8);
};

FlashSavedata$1.prototype.load32 = function(offset) {
	return (this.load8(offset) & 0xFF) | (this.load8(offset + 1) << 8) | (this.load8(offset + 2) << 16) | (this.load8(offset + 3) << 24);
};

FlashSavedata$1.prototype.loadU8 = function(offset) {
	return this.load8(offset) & 0xFF;
};

FlashSavedata$1.prototype.loadU16 = function(offset) {
	return (this.loadU8(offset) & 0xFF) | (this.loadU8(offset + 1) << 8);
};

FlashSavedata$1.prototype.store8 = function(offset, value) {
	switch (this.command) {
	case 0:
		if (offset == 0x5555) {
			if (this.second == 0x55) {
...
FlashSavedata$1.prototype.store16 = function(offset, value) {
	throw new Error("Unaligned save to flash!");
};

FlashSavedata$1.prototype.store32 = function(offset, value) {
	throw new Error("Unaligned save to flash!");
};

FlashSavedata$1.prototype.replaceData = function(memory) {
	var bank = this.view === this.bank1;
	MemoryView$1.prototype.replaceData.call(this, memory, 0);
...
function EEPROMSavedata$1(size, mmu) {
	MemoryView$1.call(this, new ArrayBuffer(size), 0);
...
EEPROMSavedata$1.prototype = Object.create(MemoryView$1.prototype);

EEPROMSavedata$1.prototype.load8 = function(offset) {
	throw new Error("Unsupported 8-bit access!");
};

EEPROMSavedata$1.prototype.load16 = function(offset) {
	return this.loadU16(offset);
};

EEPROMSavedata$1.prototype.loadU8 = function(offset) {
	throw new Error("Unsupported 8-bit access!");
};

EEPROMSavedata$1.prototype.loadU16 = function(offset) {
...
EEPROMSavedata$1.prototype.load32 = function(offset) {
	throw new Error("Unsupported 32-bit access!");
};

EEPROMSavedata$1.prototype.store8 = function(offset, value) {
	throw new Error("Unsupported 8-bit access!");
};

EEPROMSavedata$1.prototype.store16 = function(offset, value) {
	switch (this.command) {
	// Read header
...
EEPROMSavedata$1.prototype.store32 = function(offset, value) {
	throw new Error("Unsupported 32-bit access!");
};

EEPROMSavedata$1.prototype.replaceData = function(memory) {
	MemoryView$1.prototype.replaceData.call(this, memory, 0);
};

savedata.SRAMSavedata = SRAMSavedata$1;
savedata.FlashSavedata = FlashSavedata$1;
savedata.EEPROMSavedata = EEPROMSavedata$1;

var MemoryView = memoryView;
var util = util$1;
var inherit$1 = util.inherit;
var Serializer$1 = util.Serializer;
var GameBoyAdvanceGPIO = gpio.GameBoyAdvanceGPIO;
...
function MemoryBlock$1(size, cacheBits) {
...
MemoryBlock$1.prototype = Object.create(MemoryView.prototype);

MemoryBlock$1.prototype.invalidatePage = function(address) {
...
GameBoyAdvanceMMU$1.prototype.mmap = function(region, object) {
	this.memory[region] = object;
};

GameBoyAdvanceMMU$1.prototype.clear = function() {
	this.badMemory = new BadMemory(this, this.cpu);
	this.memory = [
		this.bios,
		this.badMemory, // Unused
		new MemoryBlock$1(this.SIZE_WORKING_RAM, 9),
		new MemoryBlock$1(this.SIZE_WORKING_IRAM, 7),
...
GameBoyAdvanceMMU$1.prototype.freeze = function() {
	return {
		'ram': Serializer$1.prefix(this.memory[this.REGION_WORKING_RAM].buffer),
		'iram': Serializer$1.prefix(this.memory[this.REGION_WORKING_IRAM].buffer),
	};
};

GameBoyAdvanceMMU$1.prototype.defrost = function(frost) {
	this.memory[this.REGION_WORKING_RAM].replaceData(frost.ram);
	this.memory[this.REGION_WORKING_IRAM].replaceData(frost.iram);
};

GameBoyAdvanceMMU$1.prototype.loadBios = function(bios, real) {
	this.bios = new BIOSView(bios);
	this.bios.real = !!real;
};

GameBoyAdvanceMMU$1.prototype.loadRom = function(rom, process) {
	var cart = {
		title: null,
		code: null,
		maker: null,
...
GameBoyAdvanceMMU$1.prototype.loadSavedata = function(save) {
	this.save.replaceData(save);
};

GameBoyAdvanceMMU$1.prototype.load8 = function(offset) {
	return this.memory[offset >>> this.BASE_OFFSET].load8(offset & 0x00FFFFFF);
};

GameBoyAdvanceMMU$1.prototype.load16 = function(offset) {
	return this.memory[offset >>> this.BASE_OFFSET].load16(offset & 0x00FFFFFF);
};

GameBoyAdvanceMMU$1.prototype.load32 = function(offset) {
	return this.memory[offset >>> this.BASE_OFFSET].load32(offset & 0x00FFFFFF);
};

GameBoyAdvanceMMU$1.prototype.loadU8 = function(offset) {
	return this.memory[offset >>> this.BASE_OFFSET].loadU8(offset & 0x00FFFFFF);
};

GameBoyAdvanceMMU$1.prototype.loadU16 = function(offset) {
	return this.memory[offset >>> this.BASE_OFFSET].loadU16(offset & 0x00FFFFFF);
};

GameBoyAdvanceMMU$1.prototype.store8 = function(offset, value) {
...
GameBoyAdvanceMMU$1.prototype.store16 = function(offset, value) {
...
GameBoyAdvanceMMU$1.prototype.store32 = function(offset, value) {
...
GameBoyAdvanceMMU$1.prototype.waitPrefetch = function(memory) {
	this.cpu.cycles += 1 + this.waitstatesPrefetch[memory >>> this.BASE_OFFSET];
};

GameBoyAdvanceMMU$1.prototype.waitPrefetch32 = function(memory) {
	this.cpu.cycles += 1 + this.waitstatesPrefetch32[memory >>> this.BASE_OFFSET];
};

GameBoyAdvanceMMU$1.prototype.wait = function(memory) {
	this.cpu.cycles += 1 + this.waitstates[memory >>> this.BASE_OFFSET];
};

GameBoyAdvanceMMU$1.prototype.wait32 = function(memory) {
	this.cpu.cycles += 1 + this.waitstates32[memory >>> this.BASE_OFFSET];
};

GameBoyAdvanceMMU$1.prototype.waitSeq = function(memory) {
	this.cpu.cycles += 1 + this.waitstatesSeq[memory >>> this.BASE_OFFSET];
};

GameBoyAdvanceMMU$1.prototype.waitSeq32 = function(memory) {
	this.cpu.cycles += 1 + this.waitstatesSeq32[memory >>> this.BASE_OFFSET];
};

GameBoyAdvanceMMU$1.prototype.waitMul = function(rs) {
...
GameBoyAdvanceMMU$1.prototype.waitMulti32 = function(memory, seq) {
	this.cpu.cycles += 1 + this.waitstates32[memory >>> this.BASE_OFFSET];
	this.cpu.cycles += (1 + this.waitstatesSeq32[memory >>> this.BASE_OFFSET]) * (seq - 1);
};

GameBoyAdvanceMMU$1.prototype.addressToPage = function(region, address) {
	return address >> this.memory[region].ICACHE_PAGE_BITS;
};

GameBoyAdvanceMMU$1.prototype.accessPage = function(region, pageId) {
...
GameBoyAdvanceMMU$1.prototype.scheduleDma = function(number, info) {
	switch (info.timing) {
...
GameBoyAdvanceMMU$1.prototype.runHblankDmas = function() {
	var dma;
...
GameBoyAdvanceMMU$1.prototype.runVblankDmas = function() {
	var dma;
	for (var i = 0; i < this.cpu.irq.dma.length; ++i) {
		dma = this.cpu.irq.dma[i];
...
GameBoyAdvanceMMU$1.prototype.serviceDma = function(number, info) {
	if (!info.enable) {
...
GameBoyAdvanceMMU$1.prototype.adjustTimings = function(word) {
...
GameBoyAdvanceMMU$1.prototype.saveNeedsFlush = function() {
	return this.save.writePending;
};

GameBoyAdvanceMMU$1.prototype.flushSave = function() {
	this.save.writePending = false;
};

GameBoyAdvanceMMU$1.prototype.allocGPIO = function(rom) {
	return new GameBoyAdvanceGPIO(this.core, rom);
};

mmu.MemoryBlock = MemoryBlock$1;
mmu.ROMView = ROMView;
mmu.BIOSView = BIOSView;
mmu.GameBoyAdvanceMMU = GameBoyAdvanceMMU$1;

var inherit = util$1.inherit;
var MemoryBlock = mmu.MemoryBlock;

function GameBoyAdvanceInterruptHandler$1() {
	inherit.call(this);
...
GameBoyAdvanceInterruptHandler$1.prototype.clear = function() {
	this.enable = false;
	this.enabledIRQs = 0;
...
GameBoyAdvanceInterruptHandler$1.prototype.freeze = function() {
	return {
		'enable': this.enable,
...
GameBoyAdvanceInterruptHandler$1.prototype.defrost = function(frost) {
...
GameBoyAdvanceInterruptHandler$1.prototype.updateTimers = function() {
...
GameBoyAdvanceInterruptHandler$1.prototype.resetSP = function() {
...
GameBoyAdvanceInterruptHandler$1.prototype.swi32 = function(opcode) {
	this.swi(opcode >> 16);
};

GameBoyAdvanceInterruptHandler$1.prototype.swi = function(opcode) {
...
GameBoyAdvanceInterruptHandler$1.prototype.masterEnable = function(value) {
	this.enable = value;
...
GameBoyAdvanceInterruptHandler$1.prototype.setInterruptsEnabled = function(value) {
...
GameBoyAdvanceInterruptHandler$1.prototype.pollNextEvent = function() {
...
GameBoyAdvanceInterruptHandler$1.prototype.waitForIRQ = function() {
	var timer;
...
GameBoyAdvanceInterruptHandler$1.prototype.testIRQ = function() {
	if (this.enable && this.enabledIRQs & this.interruptFlags) {
		this.springIRQ = true;
...
GameBoyAdvanceInterruptHandler$1.prototype.raiseIRQ = function(irqType) {
...
GameBoyAdvanceInterruptHandler$1.prototype.dismissIRQs = function(irqMask) {
	this.interruptFlags &= ~irqMask;
	this.io.registers[this.io.IF >> 1] = this.interruptFlags;
};

GameBoyAdvanceInterruptHandler$1.prototype.dmaSetSourceAddress = function(dma, address) {
	this.dma[dma].source = address & 0xFFFFFFFE;
};

GameBoyAdvanceInterruptHandler$1.prototype.dmaSetDestAddress = function(dma, address) {
	this.dma[dma].dest = address & 0xFFFFFFFE;
};

GameBoyAdvanceInterruptHandler$1.prototype.dmaSetWordCount = function(dma, count) {
	this.dma[dma].count = count ? count : (dma == 3 ? 0x10000 : 0x4000);
};

GameBoyAdvanceInterruptHandler$1.prototype.dmaWriteControl = function(dma, control) {
...
GameBoyAdvanceInterruptHandler$1.prototype.timerSetReload = function(timer, reload) {
	this.timers[timer].reload = reload & 0xFFFF;
};

GameBoyAdvanceInterruptHandler$1.prototype.timerWriteControl = function(timer, control) {
...
GameBoyAdvanceInterruptHandler$1.prototype.timerRead = function(timer) {
	var currentTimer = this.timers[timer];
...
GameBoyAdvanceInterruptHandler$1.prototype.halt = function() {
	if (!this.enable) {
...
GameBoyAdvanceInterruptHandler$1.prototype.lz77 = function(source, dest, unitsize) {
...
GameBoyAdvanceInterruptHandler$1.prototype.huffman = function(source, dest) {
...
GameBoyAdvanceInterruptHandler$1.prototype.rl = function(source, dest, unitsize) {
	source = source & 0xFFFFFFFC;
	var remaining = (this.cpu.mmu.load32(source) & 0xFFFFFF00) >> 8;
	var padding = (4 - remaining) & 0x3;
...
var irq = GameBoyAdvanceInterruptHandler$1;

var Serializer = util$1.Serializer;

function GameBoyAdvanceIO$1() {
	// Video
	this.DISPCNT = 0x000;
	this.GREENSWP = 0x002;
	this.DISPSTAT = 0x004;
...
GameBoyAdvanceIO$1.prototype.clear = function() {
...
GameBoyAdvanceIO$1.prototype.freeze = function() {
	return {
		'registers': Serializer.prefix(this.registers.buffer)
	};
};

GameBoyAdvanceIO$1.prototype.defrost = function(frost) {
...
GameBoyAdvanceIO$1.prototype.load8 = function(offset) {
	throw 'Unimplmeneted unaligned I/O access';
};

GameBoyAdvanceIO$1.prototype.load16 = function(offset) {
	return (this.loadU16(offset) << 16) >> 16;
};

GameBoyAdvanceIO$1.prototype.load32 = function(offset) {
	offset &= 0xFFFFFFFC;
	switch (offset) {
...
GameBoyAdvanceIO$1.prototype.loadU8 = function(offset) {
	var odd = offset & 0x0001;
	var value = this.loadU16(offset & 0xFFFE);
	return (value >>> (odd << 3)) & 0xFF;
};

GameBoyAdvanceIO$1.prototype.loadU16 = function(offset) {
	switch (offset) {
	case this.DISPCNT:
	case this.BG0CNT:
	case this.BG1CNT:
...
GameBoyAdvanceIO$1.prototype.store8 = function(offset, value) {
	switch (offset) {
	case this.WININ:
		this.value & 0x3F;
		break;
...
GameBoyAdvanceIO$1.prototype.store16 = function(offset, value) {
	switch (offset) {
	// Video
	case this.DISPCNT:
...
GameBoyAdvanceIO$1.prototype.store32 = function(offset, value) {
	switch (offset) {
	case this.BG2X_LO:
		value &= 0x0FFFFFFF;
...
GameBoyAdvanceIO$1.prototype.invalidatePage = function(address) {};

GameBoyAdvanceIO$1.prototype.STUB_REG = function(type, offset) {
	this.core.STUB('Unimplemented ' + type + ' register write: ' + offset.toString(16));
};

var io = GameBoyAdvanceIO$1;

function GameBoyAdvanceAudio$1() {
	{
		this.context = null;
	}
...
GameBoyAdvanceAudio$1.prototype.clear = function() {
	this.fifoA = [];
	this.fifoB = [];
	this.fifoASample = 0;
	this.fifoBSample = 0;
...
GameBoyAdvanceAudio$1.prototype.freeze = function() {
	return {
		nextSample: this.nextSample
	};
};

GameBoyAdvanceAudio$1.prototype.defrost = function(frost) {
	this.nextSample = frost.nextSample;
};

GameBoyAdvanceAudio$1.prototype.pause = function(paused) {
	if (this.context) {
		if (paused) {
			try {
				this.jsAudio.disconnect(this.context.destination);
...
GameBoyAdvanceAudio$1.prototype.updateTimers = function() {
...
GameBoyAdvanceAudio$1.prototype.writeEnable = function(value) {
	this.enabled = !!value;
	this.nextEvent = this.cpu.cycles;
...
GameBoyAdvanceAudio$1.prototype.writeSoundControlLo = function(value) {
...
GameBoyAdvanceAudio$1.prototype.writeSoundControlHi = function(value) {
	switch (value & 0x0003) {
	case 0:
...
GameBoyAdvanceAudio$1.prototype.resetSquareChannel = function(channel) {
	if (channel.step) {
...
GameBoyAdvanceAudio$1.prototype.setSquareChannelEnabled = function(channel, enable) {
...
GameBoyAdvanceAudio$1.prototype.writeSquareChannelSweep = function(channelId, value) {
...
GameBoyAdvanceAudio$1.prototype.writeSquareChannelDLE = function(channelId, value) {
	var channel = this.squareChannels[channelId];
...
GameBoyAdvanceAudio$1.prototype.writeSquareChannelFC = function(channelId, value) {
	var channel = this.squareChannels[channelId];
...
GameBoyAdvanceAudio$1.prototype.updateSquareChannel = function(channel, cycles) {
...
GameBoyAdvanceAudio$1.prototype.writeChannel3Lo = function(value) {
...
GameBoyAdvanceAudio$1.prototype.writeChannel3Hi = function(value) {
...
GameBoyAdvanceAudio$1.prototype.writeChannel3X = function(value) {
...
GameBoyAdvanceAudio$1.prototype.resetChannel3 = function() {
...
GameBoyAdvanceAudio$1.prototype.writeWaveData = function(offset, data, width) {
...
GameBoyAdvanceAudio$1.prototype.setChannel4Enabled = function(enable) {
...
GameBoyAdvanceAudio$1.prototype.writeChannel4LE = function(value) {
	this.writeChannelLE(this.channel4, value);
	this.resetChannel4();
};

GameBoyAdvanceAudio$1.prototype.writeChannel4FC = function(value) {
...
GameBoyAdvanceAudio$1.prototype.resetChannel4 = function() {
...
GameBoyAdvanceAudio$1.prototype.writeChannelLE = function(channel, value) {
...
GameBoyAdvanceAudio$1.prototype.updateEnvelope = function(channel, cycles) {
	if (channel.step) {
...
GameBoyAdvanceAudio$1.prototype.appendToFifoA = function(value) {
	var b;
...
GameBoyAdvanceAudio$1.prototype.appendToFifoB = function(value) {
	var b;
...
GameBoyAdvanceAudio$1.prototype.sampleFifoA = function() {
...
GameBoyAdvanceAudio$1.prototype.sampleFifoB = function() {
...
GameBoyAdvanceAudio$1.prototype.scheduleFIFODma = function(number, info) {
	switch (info.dest) {
...
GameBoyAdvanceAudio$1.prototype.sample = function() {
	var sampleLeft = 0;
	var sampleRight = 0;
	var sample;
	var channel;
...
GameBoyAdvanceAudio$1.prototype.audioProcess = function(audioProcessingEvent) {
...
var audio = GameBoyAdvanceAudio$1;
...
function GameBoyAdvanceOBJ(oam, index) {
	...
	this.pushPixel = GameBoyAdvanceSoftwareRenderer$1.pushPixel;
	this.cachedWidth = 8;
	this.cachedHeight = 8;
}
...
function GameBoyAdvanceSoftwareRenderer$1() {
	this.LAYER_BG0 = 0;
	this.LAYER_BG1 = 1;
	this.LAYER_BG2 = 2;
	this.LAYER_BG3 = 3;
...
GameBoyAdvanceSoftwareRenderer$1.prototype.clear = function(mmu) {
	...
	for (var i = 0; i < 4; ++i) {
		this.bg.push({
			...
			sx: 0,
			sy: 0,
			pushPixel: GameBoyAdvanceSoftwareRenderer$1.pushPixel,
...
GameBoyAdvanceSoftwareRenderer$1.prototype.clearSubsets = function(mmu, regions) {
	if (regions & 0x04) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.freeze = function() {
};

GameBoyAdvanceSoftwareRenderer$1.prototype.defrost = function(frost) {
};

GameBoyAdvanceSoftwareRenderer$1.prototype.setBacking = function(backing) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.writeDisplayControl = function(value) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.writeBackgroundControl = function(bg, value) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.writeBackgroundHOffset = function(bg, value) {
	this.bg[bg].x = value & 0x1FF;
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeBackgroundVOffset = function(bg, value) {
	this.bg[bg].y = value & 0x1FF;
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeBackgroundRefX = function(bg, value) {
	this.bg[bg].refx = (value << 4) / 0x1000;
	this.bg[bg].sx = this.bg[bg].refx;
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeBackgroundRefY = function(bg, value) {
	this.bg[bg].refy = (value << 4) / 0x1000;
	this.bg[bg].sy = this.bg[bg].refy;
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeBackgroundParamA = function(bg, value) {
	this.bg[bg].dx = (value << 16) / 0x1000000;
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeBackgroundParamB = function(bg, value) {
	this.bg[bg].dmx = (value << 16) / 0x1000000;
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeBackgroundParamC = function(bg, value) {
	this.bg[bg].dy = (value << 16) / 0x1000000;
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeBackgroundParamD = function(bg, value) {
	this.bg[bg].dmy = (value << 16) / 0x1000000;
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeWin0H = function(value) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.writeWin1H = function(value) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.writeWin0V = function(value) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.writeWin1V = function(value) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.writeWindow = function(index, value) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.writeWinIn = function(value) {
	this.writeWindow(0, value);
	this.writeWindow(1, value >> 8);
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeWinOut = function(value) {
	this.writeWindow(2, value);
	this.writeWindow(3, value >> 8);
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeBlendControl = function(value) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.setBlendEnabled = function(layer, enabled, override) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.writeBlendAlpha = function(value) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.writeBlendY = function(value) {
	this.blendY = value;
	this.palette.setBlendY(value >= 16 ? 1 : (value / 16));
};

GameBoyAdvanceSoftwareRenderer$1.prototype.writeMosaic = function(value) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.resetLayers = function() {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.layerComparator = function(a, b) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.accessMapMode0 = function(base, size, x, yBase, out) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.accessMapMode1 = function(base, size, x, yBase, out) {
	var offset = base + (x >> 3) + yBase;

	out.tile = this.vram.loadU8(offset);
};

GameBoyAdvanceSoftwareRenderer$1.prototype.accessTile = function(base, tile, y) {
...
GameBoyAdvanceSoftwareRenderer$1.pushPixel = function(layer, map, video, row, x, offset, backing, mask, raw) {
	var index;
	if (!raw) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.identity = function(x) {
	return x;
};

GameBoyAdvanceSoftwareRenderer$1.prototype.drawScanlineBlank = function(backing) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.prepareScanline = function(backing) {
	for (var x = 0; x < this.HORIZONTAL_PIXELS; ++x) {
		backing.stencil[x] = this.target2[this.LAYER_BACKDROP];
	}
};

GameBoyAdvanceSoftwareRenderer$1.prototype.drawScanlineBGMode0 = function(backing, bg, start, end) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.drawScanlineBGMode2 = function(backing, bg, start, end) {
	var video = this.video;
	var x;
	var y = video.vcount;
	var offset = start;
...
GameBoyAdvanceSoftwareRenderer$1.prototype.drawScanlineBGMode3 = function(backing, bg, start, end) {
	var video = this.video;
	var x;
	var y = video.vcount;
	var offset = start;
...
GameBoyAdvanceSoftwareRenderer$1.prototype.drawScanlineBGMode4 = function(backing, bg, start, end) {
	var video = this.video;
	var x;
	var y = video.vcount;
	var offset = start;
...
GameBoyAdvanceSoftwareRenderer$1.prototype.drawScanlineBGMode5 = function(backing, bg, start, end) {
	var video = this.video;
	var x;
	var y = video.vcount;
	var offset = start;
...
GameBoyAdvanceSoftwareRenderer$1.prototype.drawScanline = function(y) {
...
GameBoyAdvanceSoftwareRenderer$1.prototype.finishScanline = function(backing) {
	var color;
...
GameBoyAdvanceSoftwareRenderer$1.prototype.startDraw = function() {
	// Nothing to do
};

GameBoyAdvanceSoftwareRenderer$1.prototype.finishDraw = function(caller) {
...
software.GameBoyAdvanceSoftwareRenderer = GameBoyAdvanceSoftwareRenderer$1;

var GameBoyAdvanceSoftwareRenderer = software.GameBoyAdvanceSoftwareRenderer;

function GameBoyAdvanceVideo$1() {
...
GameBoyAdvanceVideo$1.prototype.clear = function() {
...
GameBoyAdvanceVideo$1.prototype.freeze = function() {
	return {
...
GameBoyAdvanceVideo$1.prototype.defrost = function(frost) {
...
GameBoyAdvanceVideo$1.prototype.setBacking = function(backing) {
...
GameBoyAdvanceVideo$1.prototype.updateTimers = function(cpu) {
...
GameBoyAdvanceVideo$1.prototype.writeDisplayStat = function(value) {
...
GameBoyAdvanceVideo$1.prototype.readDisplayStat = function() {
	return (this.inVblank) | (this.inHblank << 1) | (this.vcounter << 2);
};

GameBoyAdvanceVideo$1.prototype.finishDraw = function(pixelData) {
	// this.context.putImageData(pixelData, 0, 0);
	this.drawCallback();
};

var video = GameBoyAdvanceVideo$1;

function GameBoyAdvanceKeypad$1() {
...
GameBoyAdvanceKeypad$1.prototype.press = function(key, time) {
...
GameBoyAdvanceKeypad$1.prototype.keydown = function(key) {
	var toggle = key;
...
GameBoyAdvanceKeypad$1.prototype.keyup = function(key) {
	var toggle = key;
...
GameBoyAdvanceKeypad$1.prototype.keyboardHandler = function(e) {
	var toggle = 0;
	switch (e.keyCode) {
...
GameBoyAdvanceKeypad$1.prototype.gamepadHandler = function(gamepad) {
	var value = 0;
...
GameBoyAdvanceKeypad$1.prototype.gamepadConnectHandler = function(gamepad) {
	this.gamepads.push(gamepad);
};

GameBoyAdvanceKeypad$1.prototype.gamepadDisconnectHandler = function(gamepad) {
	this.gamepads = self.gamepads.filter(function(other) { return other != gamepad });
};

GameBoyAdvanceKeypad$1.prototype.pollGamepads = function() {
	var navigatorList = [];
...
GameBoyAdvanceKeypad$1.prototype.registerHandlers = function() {
...
var keypad = GameBoyAdvanceKeypad$1;

var hex = util$1.hex;

function GameBoyAdvanceSIO$1() {
	this.SIO_NORMAL_8 = 0;
...
GameBoyAdvanceSIO$1.prototype.clear = function() {
...
GameBoyAdvanceSIO$1.prototype.setMode = function(mode) {
	if (mode & 0x8) {
		mode &= 0xC;
	} else {
		mode &= 0x3;
...
GameBoyAdvanceSIO$1.prototype.writeRCNT = function(value) {
...
GameBoyAdvanceSIO$1.prototype.writeSIOCNT = function(value) {
	switch (this.mode) {
...
GameBoyAdvanceSIO$1.prototype.readSIOCNT = function() {
...
GameBoyAdvanceSIO$1.prototype.read = function(slot) {
	switch (this.mode) {
	case this.SIO_NORMAL_32:
		this.core.STUB('32-bit transfer unsupported');
		break;
...
var sio = GameBoyAdvanceSIO$1;

function MemoryCanvas$1() {
}

MemoryCanvas$1.prototype.getContext = function () {
  return this;
};

MemoryCanvas$1.prototype.createImageData = function (w, h) {
  var pixelData = {
    // RGBA
    width: w,
    height: h,
...
var memoryCanvas = MemoryCanvas$1;

var fs = require$$0$1;
var PNG = require$$1.PNG;

var ARMCore = core;
var GameBoyAdvanceMMU = mmu.GameBoyAdvanceMMU;
var GameBoyAdvanceInterruptHandler = irq;
var GameBoyAdvanceIO = io;
var GameBoyAdvanceAudio = audio;
var GameBoyAdvanceVideo = video;
var GameBoyAdvanceKeypad = keypad;
var GameBoyAdvanceSIO = sio;
